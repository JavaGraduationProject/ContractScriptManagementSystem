<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="剧本类型" prop="jubenleixing">
          <el-select v-model="ruleForm.jubenleixing" placeholder="请选择剧本类型">
            <el-option
                v-for="(item,index) in jubenleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jubenleixing/leixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jubenleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `juben/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jubenCrossAddOrUpdateFlag = false;
                  this.parent.search();
