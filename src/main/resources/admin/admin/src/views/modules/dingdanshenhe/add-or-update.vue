<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="游戏时间" prop="youxishijian">
          <el-input v-model="ruleForm.youxishijian" 
              placeholder="游戏时间" clearable  :readonly="ro.youxishijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="游戏时间" prop="youxishijian">
              <el-input v-model="ruleForm.youxishijian" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="房间号" prop="fangjianhao">
          <el-select v-model="ruleForm.fangjianhao" placeholder="请选择房间号">
            <el-option
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/fangjian/fangjianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
            ...
            this.zhuangtaiOptions = "成功,失败".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `dingdanshenhe/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dingdanshenhe/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.dingdanshenheCrossAddOrUpdateFlag = false;
                  this.parent.search();
